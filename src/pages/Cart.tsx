...
export function Cart() {
 ...
 <h2 className="text-3xl font-serif text-theme-text mb-4">Your basket is empty!</h2>
 <p className="text-theme-muted mb-8 font-light">Looks like you haven't added any cute crochets yet.</p>
 <Link to="/shop" className="inline-flex bg-theme-text text-white px-8 py-3 rounded-full font-medium shadow-sm hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 transition-all text-center justify-center w-max mx-auto">
 Start Shopping
 </Link>
 </div>
 </div>
 ...
 <h1 className="text-2xl font-serif text-theme-text mb-6">Review Items</h1>
 <div className="space-y-4 overflow-y-auto">
 {items.map(item => (
 <div key={item.id} className="flex items-center gap-4 group bg-theme-bg p-3 rounded-2xl border hover:border-gray-200 transition-colors">
 <div className="w-20 h-20 bg-theme-light-pink-box rounded-xl flex-shrink-0 overflow-hidden relative">
 <img src={item.imageUrl} alt={item.title} className="absolute inset-0 w-full h-full object-cover mix-blend-multiply" />
 </div>
 <div className="flex-1">
 <div className="text-base font-medium text-theme-text">{item.title}</div>
 <div className="text-sm text-theme-muted mt-1">${item.price.toFixed(2)}</div>
 </div>
 ...
 <span className="font-medium w-4 text-center text-sm text-theme-text">{item.quantity}</span>
 <button 
 onClick={() => updateQuantity(item.id, item.quantity + 1)}
 ...
 <span className="bg-theme-main text-theme-muted text-xs px-2 py-1 font-medium rounded-full tracking-wide">EST. 2 DAYS</span>
 </div>
 
 <div className="border-t border-gray-100 pt-6 space-y-4">
 <div className="flex justify-between text-sm">
 <span className="text-theme-muted">Subtotal</span>
 <span className="font-medium text-theme-text">${total().toFixed(2)}</span>
 </div>
 <div className="flex justify-between text-sm">
 ...
 <span className="text-theme-brand font-medium">${total().toFixed(2)}</span>
 </div>
 <Link 
 to="/checkout"
