...
async function startServer() {
  ...
  app.post("/api/products", upload.single("image"), (req, res) => {
    ...
    const stmt = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
    const info = stmt.run(title, description, parseFloat(price), imageUrl, category, parseInt(stock || "1", 10));
  ...
  app.post("/api/orders", (req, res) => {
    const { customerName, email, address, items, total, paymentMethod } = req.body;
    const trackingId = "TRK" + Date.now() + Math.random().toString(36).substring(2, 6).toUpperCase();
    
    // Using a transaction to insert order and items
    const insertTransaction = db.transaction((orderData, orderItems) => {
      const stmt = db.prepare("INSERT INTO orders (trackingId, customerName, email, address, paymentMethod, total) VALUES (?, ?, ?, ?, ?, ?)");
      const info = stmt.run(orderData.trackingId, orderData.customerName, orderData.email, orderData.address, orderData.paymentMethod, orderData.total);
    ...
    try {
      const orderId = insertTransaction({ trackingId, customerName, email, address, paymentMethod, total }, items);
  ...
  // Get orders (for simple order system tracking)
  app.get("/api/orders", (req, res) => {
    const orders = db.prepare("SELECT * FROM orders ORDER BY createdAt DESC").all() as any[];
    const items = db.prepare("SELECT oi.*, p.title, p.imageUrl FROM order_items oi JOIN products p ON oi.productId = p.id").all() as any[];
  ...
  app.get("/api/orders/track/:trackingId", (req, res) => {
    const order = db.prepare("SELECT * FROM orders WHERE trackingId = ?").get(req.params.trackingId) as any;
    if (!order) return res.status(404).json({ error: "Order not found" });
    const items = db.prepare("SELECT oi.*, p.title, p.imageUrl FROM order_items oi JOIN products p ON oi.productId = p.id WHERE orderId = ?").all(order.id) as any[];
  ...
  app.listen(PORT, "0.0.0.0", () => {
    ...
    if (count.count === 0) {
      db.prepare(`INSERT INTO products (title, description, price, imageUrl, category, stock) VALUES 
